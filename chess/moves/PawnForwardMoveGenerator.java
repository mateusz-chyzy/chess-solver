...
public class PawnForwardMoveGenerator extends AbstractMoveGenerator {
    ...
    public PawnForwardMoveGenerator(MoveDelta moveDelta, Rank promotionRank) {
    ...
    public List<AbstractMoveStrategy> getPossibleMoves(Piece movingPiece, Board board) {
        ...
        if (nextPosition.isPresent()) {
            ...
            if (board.getPieceAt(pos).isEmpty()) {
                ...
                if (move.initialPosition().rank() == promotionRank) {
                    for (ChessPiece piece : promotionPieces) {
                        possibleMoves.add(new PawnPromotionMoveStrategy(board, move, piece));
                    }
                } else {
                    possibleMoves.add(new SimpleMoveStrategy(board, move));
                }
            }
        }
